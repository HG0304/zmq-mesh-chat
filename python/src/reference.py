...
def handle_register(
    ...
    res.reference_timestamp_ms = current_ms
...
def handle_list(
    ...
    res.ok = True
    for server_name, rank in sorted(ranks_by_name.items(), key=lambda item: item[1]):
...
def main() -> None:
    ...
    while True:
        ...
        current_ms = now_ms()
        prune_stale(heartbeat_ts_by_name, current_ms, ttl_ms)
